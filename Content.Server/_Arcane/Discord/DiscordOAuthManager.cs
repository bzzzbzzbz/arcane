...
using Content.Shared._Arcane.CCVar;
using Content.Shared._Arcane.Discord;
using Robust.Shared.Configuration;
using Robust.Shared.Network;
using Robust.Shared.Player;

namespace Content.Server._Arcane;
...
public sealed class DiscordOAuthManager : IDiscordOAuthManager, IDisposable
{
    ...
    {
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _cfg.GetCVar(ArcaneCCvar.AuthApiToken));
    ...
    {
        if (_cfg.GetCVar(ArcaneCCvar.AuthApiUrl) is not { } apiUrl || string.IsNullOrEmpty(apiUrl))
            return [];
        var url = new Uri(apiUrl);
        var guild = _cfg.GetCVar(ArcaneCCvar.AuthTargetGuild);
